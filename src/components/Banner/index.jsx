		...
		<section id='banner' className='banner'>
			...
			<div> 
				{slideArray.map((items, index) => (
					<div key={index}>
						{<img className=
						{ slideArray[currentState +1].img ? 'banner__picture banner__picture--in' : 'banner__picture banner__picture--out'} 
						src={slide} 
						alt='slide' />}
					</div>
				))}
			...
			<div> 
				{slideArray2.map((items, index) => (
					<div key={index}>
						{<img className=
						{ slideArray[currentState +1].img ? 'banner__picture2 banner__picture--in' : 'banner__picture2 banner__picture--out'}
						src={slide2} 
						alt='slide' />}
					</div>
				))}
